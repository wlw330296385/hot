...
$('.reg_user').blur(function () {
    var gName = $.trim($(".reg_user").val());
    $(".reg_user").val(gName);
    $('.validate-result span').remove('.user_yz');
    if(((/\s/).test(gName))===true)
    {
        $('.validate-result').append('<span class="user_yz">填写提示:会员名不许有空格</span>');
    }else 
    // if ((/^[a-zA-Z0-9]{6,15}$/).test($(".reg_user").val())==false && ((/^[\u4E00-\u9FA5]{2,5}$/).test($(".reg_user").val())===false) ) 
    if((/^[\u4E00-\u9FA5A-Za-z0-9_]{2,15}$/).test(gName)===false)
    {
        $('.validate-result').append('<span class="user_yz">提示:会员名只能包含英文、数字或中文，长度为2-15位</span>');
    } 
...
$('.reg_password').blur(function () {
    ...
    if ((/[a-z0-9_-]|[!@#$%^&*()~]{6,16}/).test($(".reg_password").val())==false) {
        $('.validate-result').append('<span class="password_yz">填写提示:密码长度为6-16位</span>');
    }
});
...
$('.reg_confirm').blur(function () {
    ...
    if (($(".reg_password").val()) != ($(".reg_confirm").val())) {
        $('.validate-result').append('<span class="confirm_yz">填写提示:两次密码输入不一致</span>');
    }
});
...
$('.reg_email').blur(function () {
    ...
    if ((/^[a-z\d]+(\.[a-z\d]+)*@([\da-z](-[\da-z])?)+(\.{1,2}[a-z]+)+$/).test($(".reg_email").val())==false) {
        $('.validate-result').append('<span class="email_yz">填写提示:两次密码输入不一致</span>');
    }
});
...
$('.reg_mobile').blur(function () {
    ...
    if ((/^1[34578]\d{9}$/).test($(".reg_mobile").val())==false) {
        $('.validate-result').append('<span class="mobile_yz">填写提示:请输入正确的手机格式</span>');
    }
});
