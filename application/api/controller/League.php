...
class League extends Base
{
    ...
    public function creatematch()
    {
        ...
        if (!$data['min_teammembers']) {
            switch ($data['format']) {
                case '1v1':
                    $data['min_teammembers'] = 1;
                    break;
                case '2v2':
                    $data['min_teammembers'] = 2;
                    break;
                case '3v3':
                    $data['min_teammembers'] = 3;
                    break;
                case '4v4':
                    $data['min_teammembers'] = 4;
                    break;
                case '5v5':
    ...
    public function platformleaguelist()
    {
        try {
            ...
            // 获取联赛列表
            $leagueS = new LeagueService();
            $res = $leagueS->getLeagueListWithTeamsCount($data, $page, 'start_time desc');
    ...
    public function platformleaguepage()
    {
        try {
            ...
            // 获取联赛列表
            $matchS = new MatchService();
            $result = $matchS->getLeagueListWithTeamsCount($data);
            if (!$result) {
                $response = ['code' => 100, 'msg' => __lang('MSG_000')];
            } else {
        ...
        try {
            ...
            if ($result['code'] == 200) {
                ...
                if ($data['status'] == 2) {
                    ...
                    if (  $matchApply['match']['teams_max']  && $matchTeamCount == $matchApply['match']['teams_max']) {
...
// 保存联赛球队分组数据
    ...
    public function getmatchmembers()
    {
        ...
        if (empty($myMatchMember) || $myMatchMember['type'] < 9) {
            return json(['code' => 100, 'msg' => __lang('MSG_403')]);
        }
    ...
    // 联赛工作人员列表
    public function getMatchMembersWithTag()
    {
        try {
            $data = input('param.');
            if (input('?param.league_id')) {
            ...
            if (!$result) {
                ...
                $tempArray = [];
                foreach ($result as $row) {
                    if (empty($tempArray[$row['member_id']])) {
                        $tempArray[$row['member_id']] = $row;
                        $tempArray[$row['member_id']]['job_description_arr'] = [];
                        $tempArray[$row['member_id']]['role_arr'] = [];
                        $tempArray[$row['member_id']]['role_arr'][0] = $row['type_text'];
                        if (!empty($row['custom_role'])) {
                            $tempArray[$row['member_id']]['role_arr'][1] = $row['custom_role'];
                        }
                        if (!empty($row['job_description'])) {
                            $tempArray[$row['member_id']]['job_description_arr'][0] = $row['job_description'];
                        }
                    } else {
                        array_push($tempArray[$row['member_id']]['role_arr'], $row['type_text']);
                        if (!empty($row['custom_role'])) {
                            array_push($tempArray[$row['member_id']]['role_arr'], $row['custom_role']);
                        }
                        if (!empty($row['job_description'])) {
                            array_push($tempArray[$row['member_id']]['job_description_arr'], $row['job_description']);
                        }
                    }
                }
                $matchMembers = [];
                foreach ($tempArray as $row) {
                    array_push($matchMembers, $row);
                }
                return json(['code' => 200, 'msg' => __lang('MSG_201'), 'data' => $matchMembers]);
            }
        } catch (Exception $e) {
            trace($e->getMessage(), 'error');
            return json(['code' => 100, 'msg' => __lang('MSG_404')]);
    ...
    public function checkmatchmember()
    {
        try {
            ...
            if ($matchorgmember) {
                if ($matchorgmember['status'] == 1) {
                    // 会员已是正式联赛工作人员
                    return json(['code' => 100, 'msg' => '会员已是正式联赛工作人员']);
                } else if ($matchorgmember['status'] == 0) {
                    // 有数据，不是正式数据
                    return json(['code' => 100, 'msg' => '已邀请会员']);
                }
            } else {
                // 没有联赛组织人员数据
                return json(['code' => 100, 'msg' => __lang('MSG_000')]);
    ...
    public function invitematchmebmer()
    {
        ...
        if (!is_array($data['type'])) {
            return json(['code' => 100, 'msg' => 'type参数不正确']);
        }
        ...
        foreach ($data['type'] as $row) {
            ...
            $type_temp = intval($row['type']);
            $typeStr = $typesArr[$type_temp];
    ...
    // 批量修改联赛工作人员的权限, 支持增加
    public function updateMatchMembers()
    {   
        $data = input('post.');
        ...
        $role_list = json_decode($data['role_list'], true);
        if (empty($role_list)) {
        ...
        // 查询你在该联赛的角色列表
        $matchMemberList = $leagueS->getMatchMembers(['member_id' => $data['member_id'],  'match_id' => $data['match_id'], 'status' => 1]);
        if (!$matchMemberList) {
            return json(['code' => 100, 'msg' => __lang('MSG_404')]);
        }
        ...
        unset($matchMemberTemplate['update_time']);
        $finalArr = [];
        ...
        foreach ($role_list as $row) {
            ...
            // 创建角色一定要带type
            if (empty($row['id']) && !isset($row['type'])) {
                return json(['code' => 100, 'msg' => "此条数据不完整"]);
            }
            ...
            if (isset($row['type'])) {
                if (!empty($row['id']) && $row['id'] == $myMatchMember['id'] && $row['type'] != $myMatchMember['type']) {
                    ...
                    // 如果为裁判员，先要检查资质
                    $refereeS = new RefereeService();
                    $refereeInfo = $refereeS->getReferee(['member_id' => $data['member_id'], 'status' => 1]);
            ...
            if (!empty($row['id'])) {
                ...
                //检查id是否在内
                if (!$id_check_flag) {
                    return json(['code' => 100, 'msg' => "错误的id"]);
                }

                // 如果修改别人的信息 需要管理员权限，并且对方不是管理员以上
                if ($row['id'] != $myMatchMember['id']) {
                    if ($row['type'] >= 9 && $myMatchMember['type'] <= 9) {
                        return json(['code' => 100, 'msg' => "修改失败，权限不足"]);
                    } else if ($myMatchMember['type'] < 9) {
                        return json(['code' => 100, 'msg' => "权限不足，需要管理员以上权限"]);
                    }
                ...
                foreach ($matchMemberList as $matchMember) {
                    if ( $row['type'] == $matchMember['type']) {
                        return json(['code' => 100, 'msg' => "你已经有相同权限，无需重复创建"]);
                    }
                }
                // 创建的联赛工作人员信息
                if ($myMatchMember['type'] < 9) {
                    return json(['code' => 100, 'msg' => "权限不足，需要管理员以上权限"]);
                }
            }
        ...
        // 修改联赛工作人员数据
        $resultUpdate = $leagueS->saveAllMatchMember($finalArr);
        if (!$resultUpdate) {
            return json(['code' => 100, 'msg' => __lang('MSG_400')]);
        } else {
            return json(['code' => 200, 'msg' => __lang('MSG_200')]);
    ...
    {   
        ...
        // 如果修改别人的信息 需要管理员权限，并且对方不是管理员以上
        if ($matchMemberInfo['id'] != $myMatchMember['id']) {
            if ($matchMemberInfo['type'] >= 9 && $myMatchMember['type'] <= 9) {
                return json(['code' => 100, 'msg' => "修改失败，权限不足"]);
            } else if ($myMatchMember['type'] < 9) {
                return json(['code' => 100, 'msg' => "权限不足，需要管理员以上权限"]);
            }
...
//     }
    ...
    public function getshowmatchschedulelist()
    {
        try {
            ...
            if (input('?param.match_group_id')){
                $match_group_id = input('param.match_group_id');
            }
            ...
            // 根据比赛时间（年月日）对数据分片
            foreach ($result as $key => $value) {
                $timestamp = strtotime($value['match_time']);
                $date = !empty($value['match_time']) ? date('Y-m-d', $timestamp) : 0;
                $value["s_num"] = $s_num;
                $s_num++;

                if ( !empty($match_group_id) && $match_group_id != $value["match_group_id"]) {
                    continue;
                } 

                if ($sort == 1) { 
                    $startTimeStamp = strtotime("-3 days");
                    $endTimeStamp = strtotime("+3 days");
                    $startDate = getStartAndEndUnixTimestamp(date('Y', $startTimeStamp), date('m', $startTimeStamp), date('d', $startTimeStamp));
                    $endDate = getStartAndEndUnixTimestamp(date('Y', $endTimeStamp), date('m', $endTimeStamp), date('d', $endTimeStamp));
                    if ($timestamp < $startDate['start'] || $timestamp > $endDate['end']) {
                        continue;
                    }
                ...
                $result1[$value['match_stage'] . '|' . $date][] = $value;
            ...
            foreach ($result1 as $key => $value) {
                ...
                $_array['date'] = ($datetime) ? date('n月d', $datetime) : '';
                $_array['day'] = ($datetime) ? $dayArr[date('w', $datetime)] : '';
                $_array['match_stage'] = $keyExplode[0];
                $_array['schedules'] = $value;
                array_push($result2, $_array);
            }

            return json(['code' => 200, 'msg' => __lang('MSG_201'), 'data' => $result2]);
...
//                ]);
    ...
    public function saveMatchHonor() {
        $data = input('post.');
        if (empty($data["league_id"]) || empty($data["type"]) || empty($data["name"])) {
            return json(['code' => 100, 'msg' => __lang('MSG_402')]);
        } else {
            $data['match_id'] = $data["league_id"];
            unset($data['league_id']);
        ...
        // 如果是修改联赛奖项信息
        if (!empty($data["id"])) {
            $matchHonor = $leagueS->getMatchHonor(['id' => $data['id'], 'match_id' => $data['match_id']]);
            if (empty($matchHonor)) {
                return json(['code' => 100, 'msg' => __lang('MSG_404')]);
            }

            if ($matchHonor['type'] != $data['type']) {
                $flagReset = 1;
            }
        }

        $updateId = $leagueS->saveMatchHonor($data);
        ...
        if ( ( !empty($data['team_member_list']) && !empty(json_decode($data['team_member_list'])) )
            ...
            $honorTeamIdArray = [];
            $honorTeamMemberIdArray = [];
            foreach ($matchHonorMemberList as $row) {
                array_push($honorTeamIdArray, $row['team_id']);
                array_push($honorTeamMemberIdArray, $row['team_member_id']);
            }
            $honorTeamIdArray = array_unique($honorTeamIdArray);
            ...
            switch ($data['type']) {
                case '1':
                    ...
                    $data['team_member_list'] = json_decode($data['team_member_list'], true);

                    // 数量不同 肯定重置
                    if (count($honorTeamMemberIdArray) != count($data['team_member_list']) ) {
                        $flagReset = 1;
                    }
                    foreach ($data['team_member_list'] as $row) {
                        // 确定该队员参加了联赛
                        $matchTeamMember = $leagueS->getMatchTeamMember([
                            'match_id' => $data['match_id'], 
                            'team_id' => $row['team_id'], 
                        ...
                        if (empty($matchTeamMember)) {
                            return json(['code' => 100, 'msg' => $row['name'].' 该队员未参加该赛事']);
                        }
                        $temp = [
                ...
                case '2':
                    ...
                    if (empty($data['team_list'])) {
                        return json(['code' => 100, 'msg' => '此为团队奖项，获奖队伍不能为空']);
                    }

                    $data['team_list'] = json_decode($data['team_list'], true);

                    if (count($honorTeamIdArray) != count($data['team_list']) ) {
                        $flagReset = 1;
                    }

                    foreach ($data['team_list'] as $row) {
                        $matchTeam = $leagueS->getMatchTeam([
                            'match_id' => $data['match_id'], 
                            'team_id' => $row['team_id']
                        ]);
                        if (empty($matchTeam)) {
                        ...
                        // 看看参赛队伍的队员，有则全部发奖，没有则记录一条
                        $matchTeamMemberList = $leagueS->getMatchTeamMemberList([
                            'match_id' => $data['match_id'], 
                            'team_id' => $row['team_id']
                        ]);
            ...
            try {
                if ($flagReset) {
                    $res1 = $leagueS->delMatchHonorMember([
                        "match_id" => $data["match_id"],
                        "match_honor_id" => $data["id"]
                    ]);
                    $res2 = $leagueS->saveAllMatchHonorMember($finalData);
                }
                Db::commit();
            } catch (\Exception $e) {
                Db::rollback();
