    ...
    public function createApply($data){
        ...
        if($result){
            if ($data['type'] == 1 && $data['organization_type'] == 1) {
                ...
                $MessageService = new \app\service\MessageService;
                $MessageService->saveMessageMemberInfo(['tital'=>'邀请加入成功','content'=>"您邀请{$data['member']}加入{$data['organization']}成功",'member_id'=>$data['inviter_id'],'member'=>$data['inviter'],'url'=>url('frontend/camp/campInfo',['camp_id'=>$data['organization_id']])]);
            }
