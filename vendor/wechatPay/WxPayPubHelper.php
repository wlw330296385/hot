...
class Common_util_pub
{
	...
	function __construct() {
		...
		$this->appid 	 = 'wxb22cfb38b4b95439';
		$this->mchid 	 = '1483498322';
		$this->key  	 = '93a793a3b611f19008432d0032251f34';
		$this->appsecret = 'd21eae1cc6d146026b6bf39d6e201796';
	}
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		//curl_setopt($ch,CURLOPT_SSLCERT,'D:\wxcert\apiclient_cert.pem');
		curl_setopt($ch,CURLOPT_SSLCERT,__DIR__.'/cacert/apiclient_cert.pem');
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		//curl_setopt($ch,CURLOPT_SSLKEY, 'D:\wxcert\apiclient_key.pem');
		curl_setopt($ch,CURLOPT_SSLKEY, __DIR__.'/cacert/apiclient_key.pem');
		//CA证书
		//curl_setopt($ch, CURLOPT_CAINFO, 'D:\wxcert\rootca.pem');
		curl_setopt($ch, CURLOPT_CAINFO, __DIR__.'/cacert/rootca.pem');
		//curl_setopt($ch, CURLOPT_CAINFO, __DIR__'/www/wwwroot/wtcsding.xcsv.cn/API/Vendor/WxPayPubHelper/cacert/rootca.pem');
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
...
class RedPack_pub extends Wxpay_client_pub
{
	...
	function createXml()
	{
		try
		{
			...
			{
				throw new SDKRuntimeException("缺少红包接口必填参数mch_billno【商户订单号】！"."<br>");
			}elseif($this->parameters["send_name"] == null){
				throw new SDKRuntimeException("缺少红包接口必填参数send_name【商户名称】！"."<br>");
			}elseif($this->parameters["re_openid"] == null){
				throw new SDKRuntimeException("缺少红包接口必填参数re_openid【用户openid】！"."<br>");
			}elseif ($this->parameters["total_amount"] == null ) {
				throw new SDKRuntimeException("缺少红包接口必填参数total_amount【付款金额】！"."<br>");
			}elseif ($this->parameters["total_num"] == null ) {
				throw new SDKRuntimeException("缺少红包接口必填参数total_num【红包发放总人数】！"."<br>");
			}elseif ($this->parameters["wishing"] == null ) {
				throw new SDKRuntimeException("缺少红包接口必填参数wishing【红包祝福语】！"."<br>");
			}elseif ($this->parameters["act_name"] == null ) {
				throw new SDKRuntimeException("缺少红包接口必填参数act_name【活动名称】！"."<br>");
			}elseif ($this->parameters["remark"] == null) {
				throw new SDKRuntimeException("缺少红包接口必填参数remark【备注】！"."<br>");
			}
...
class JsApi_pub extends Common_util_pub
{
	 ...
	 * 	作用：生成可以获得code的url
	 */
	function createOauthUrlForCode($redirectUrl,$scope = null)
	{
		$urlObj["appid"] = $this->appid;
		$urlObj["redirect_uri"] = "$redirectUrl";
		$urlObj["response_type"] = "code";
		$urlObj["scope"] = $this->scope = $scope?$scope:$this->scope;
	...
	/**
	 * 	作用：通过curl向微信提交code，以获取userinfo
	 */
	function getUserInfo()
	{
		$url = $this->createOauthUrlForOpenid();
		$oauth = $this->getCurlJson($url);
		$urlObj["openid"] = $oauth['openid'];
		$urlObj["access_token"] =  $oauth['access_token'];
		$bizString = $this->formatBizQueryParaMap($urlObj, false);
		$info_url = 'https://api.weixin.qq.com/sns/userinfo?'.$bizString;
		return $this->getCurlJson($info_url);
	}
